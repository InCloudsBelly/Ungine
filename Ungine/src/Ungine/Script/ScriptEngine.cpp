...
namespace U
{
	...
	{
		U_CORE_ASSERT(Handle, "Entity has not been instantiated!");
	...
	void ScriptEngine::LoadUngineRuntimeAssembly(const std::string& path)
	{
		MonoDomain* domain = nullptr;
		bool cleanup = false;
		if (s_MonoDomain)
		{
			domain = mono_domain_create_appdomain("Ungine Runtime", nullptr);
	...
	{
		LoadUngineRuntimeAssembly(path);
	...
	void ScriptEngine::Init(const std::string& assemblyPath)
	{
		...
		LoadUngineRuntimeAssembly(s_AssemblyPath);
	}
	...
	{
		U_CORE_ASSERT(s_EntityInstanceMap.find(dst) != s_EntityInstanceMap.end());
		U_CORE_ASSERT(s_EntityInstanceMap.find(src) != s_EntityInstanceMap.end());
		...
		for (auto& [entityID, entityInstanceData] : srcEntityMap)
		{
			for (auto& [moduleName, srcFieldMap] : srcEntityMap[entityID].ModuleFieldMap)
			{
				...
				{
					U_CORE_ASSERT(dstModuleFieldMap.find(moduleName) != dstModuleFieldMap.end());
					auto& fieldMap = dstModuleFieldMap.at(moduleName);
					U_CORE_ASSERT(fieldMap.find(fieldName) != fieldMap.end());
	...
	{
		U_CORE_ASSERT(s_EntityInstanceMap.find(sceneID) != s_EntityInstanceMap.end());
		auto& entityMap = s_EntityInstanceMap.at(sceneID);
		U_CORE_ASSERT(entityMap.find(entityID) != entityMap.end());
	...
	void ScriptEngine::InitScriptEntity(Entity entity)
	{
		...
		{
			U_CORE_ERROR("Entity references non-existent script module '{0}'", moduleName);
			return;
		}
	...
	EntityInstanceData& ScriptEngine::GetEntityInstanceData(UUID sceneID, UUID entityID)
	{
		...
		U_CORE_ASSERT(entityIDMap.find(entityID) != entityIDMap.end(), "Invalid entity ID!");
	...
	static uint32_t GetFieldSize(FieldType type)
	{
		...
		U_CORE_ASSERT(false, "Unknown field type!");
		return 0;
	}
