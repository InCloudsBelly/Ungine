...
namespace U {
	...
	static GLenum UngineToOpenGLTextureFormat(TextureFormat format)
	{
		switch (format)
		{
			case U::TextureFormat::RGB:     return GL_RGB;
			case U::TextureFormat::RGBA:    return GL_RGBA;
			case U::TextureFormat::Float16: return GL_RGBA16F;
		}
		U_CORE_ASSERT(false, "Unknown texture format!");
		return 0;
	}
		...
		Renderer::Submit([instance]() mutable
			{
				...
				glTexImage2D(GL_TEXTURE_2D, 0, UngineToOpenGLTextureFormat(instance->m_Format), instance->m_Width, instance->m_Height, 0, 
					UngineToOpenGLTextureFormat(instance->m_Format), GL_UNSIGNED_BYTE, nullptr);
			});
		...
		:m_FilePath(path)
	{
		...
		{
			U_CORE_INFO("Loading HDR texture {0}, srgb={1}", path, srgb);
		...
		{
			U_CORE_INFO("Loading texture {0}, srgb={1}", path, srgb);
			m_ImageData.Data = stbi_load(path.c_str(), &width, &height, &channels, srgb ? STBI_rgb : STBI_rgb_alpha);
			U_CORE_ASSERT(m_ImageData.Data, "Could not read image!");
		...
		Renderer::Submit([instance, srgb]() mutable
			{
				...
				else
				{
					...
					GLenum internalFormat = UngineToOpenGLTextureFormat(instance->m_Format);
					GLenum format = srgb ? GL_SRGB8 : (instance->m_IsHDR ? GL_RGB : UngineToOpenGLTextureFormat(instance->m_Format)); // HDR = GL_RGB for now
	...
	void OpenGLTexture2D::Unlock()
	{
		...
		Renderer::Submit([instance]() {
			glTextureSubImage2D(instance->m_RendererID, 0, 0, 0, instance->m_Width, instance->m_Height, UngineToOpenGLTextureFormat(instance->m_Format), GL_UNSIGNED_BYTE, instance->m_ImageData.Data);
			});
	...
	{
		U_CORE_ASSERT(m_Locked, "Texture must be locked!");
		m_ImageData.Allocate(width * height * Texture::GetBPP(m_Format));
		#if U_DEBUG
	...
	OpenGLTextureCube::OpenGLTextureCube(TextureFormat format, uint32_t width, uint32_t height)
	{
		...
		Renderer::Submit([instance, levels]() mutable
			...
			glTextureStorage2D(instance->m_RendererID, levels, UngineToOpenGLTextureFormat(instance->m_Format), instance->m_Width, instance->m_Height);
			glTextureParameteri(instance->m_RendererID, GL_TEXTURE_MIN_FILTER, levels > 1 ? GL_LINEAR_MIPMAP_LINEAR : GL_LINEAR);
			glTextureParameteri(instance->m_RendererID, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
			...
			// glTextureParameterf(instance->m_RendererID, GL_TEXTURE_MAX_ANISOTROPY, 16);
			});
	}
	...
	OpenGLTextureCube::OpenGLTextureCube(const std::string& path)
	{
		...
		U_CORE_ASSERT(faceWidth == faceHeight, "Non-square faces!");
		...
		Renderer::Submit([instance, faceWidth, faceHeight, faces]() mutable
			{
				...
				auto format = UngineToOpenGLTextureFormat(instance->m_Format);
