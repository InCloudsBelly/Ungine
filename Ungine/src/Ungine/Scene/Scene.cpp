...
namespace U
{
	...
	static void OnScriptComponentConstruct(entt::registry& registry, entt::entity entity)
	{
		...
		U_CORE_ASSERT(scene->m_EntityIDMap.find(entityID) != scene->m_EntityIDMap.end());
		...
		: m_DebugName(debugName)
	{
		...
		U_CORE_ASSERT(physxWorld.World);
	...
	void Scene::OnUpdate(Timestep ts)
	{
		...
		constexpr float stepSize = 0.01666666f;
	...
	void Scene::OnRenderRuntime(Timestep ts)
	{
		...
		U_CORE_ASSERT(cameraEntity, "Scene does not contain any cameras!");
...

		{
			...
			for (auto entity : view)
			{
				...
				if (e.HasComponent<RigidBody2DComponent>())
				{
					...
					U_CORE_ASSERT(rigidBody2D.RuntimeBody);
...

		{
			...
			for (auto entity : view)
			{
				...
				if (e.HasComponent<RigidBodyComponent>())
				{
					...
					U_CORE_ASSERT(rigidBody.RuntimeActor);
	...
	Entity Scene::CreateEntityWithID(UUID uuid, const std::string& name, bool runtimeMap)
	{
		...
		U_CORE_ASSERT(m_EntityIDMap.find(uuid) == m_EntityIDMap.end());
