...
namespace U {
	...
	{
		std::string title = sceneName + " - Ungine - " + Application::GetPlatformName() + " (" + Application::GetConfigurationName() + ")";
	...
	void EditorLayer::OpenScene()
	{
		...
		std::string filepath = app.OpenFile("Ungine Scene (*.hsc)\0*.hsc\0");
		if (!filepath.empty())
		{
	...
	{
		auto& app = Application::Get();
		std::string filepath = app.SaveFile("Ungine Scene (*.hsc)\0*.hsc\0");
		if (!filepath.empty())
		{
			SceneSerializer serializer(m_EditorScene);
	...
	bool EditorLayer::OnKeyPressedEvent(KeyPressedEvent& e)
	{
		...
		if (Input::IsKeyPressed(U_KEY_LEFT_CONTROL))
		{
			switch (e.GetKeyCode())
			{
			case KeyCode::B:
			...
			if (Input::IsKeyPressed(U_KEY_LEFT_SHIFT))
			{
				switch (e.GetKeyCode())
				{
				case KeyCode::S:
	...
	bool EditorLayer::OnMouseButtonPressed(MouseButtonPressedEvent& e)
	{
		...
		if (e.GetMouseButton() == U_MOUSE_BUTTON_LEFT && !Input::IsKeyPressed(KeyCode::LeftAlt) && !ImGuizmo::IsOver() && m_SceneState != SceneState::Play)
		{
